...
def load_params_from_candidates(explicit: Optional[Path] = None) -> Optional[Params]:
    if explicit is not None:
        ...
        if p.is_dir():
            ...
            p = cands[0]
        try:
            return Params.from_dict(json.loads(p.read_text()))
        except Exception:
            return None
...
def _corner_audit_one(spec: dict) -> Tuple[int, int]:
    ...
    our_ai = MinimaxAI(depth=depth, params=our_params)
    if opp_kind == "heuristic":
        opp_ai = MinimaxAI(depth=depth, params=None)
    elif opp_kind == "random":
        opp_ai = RandomAI()
    else:
        opp_ai = MinimaxAI(depth=depth, params=opp_params)
...
def main():
    ...
    ap.add_argument("--workers", type=int, default=1, help="Parallel workers for evaluation (processes)")
    args = ap.parse_args()
    ...
    if args.progress:
        print(
            f"[eval] Start: matches={cfg.matches}, depth={cfg.depth}, random_openings={cfg.random_openings}, opponent={args.opponent}",
            flush=True,
        )
    res = evaluate(
        params,
